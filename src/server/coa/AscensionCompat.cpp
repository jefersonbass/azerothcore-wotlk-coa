...
public:
    ...
    void OnSpellCast(Spell* spell) const
    {
             ...
             AscensionCompatData::NativePowerGainRules)
        {
            ...
            player->ModifyPower(static_cast<Powers>(rule.PowerType),
                rule.InternalAmount + NativePowerBonus(player, rule));
        }
        ...
        uint32 damage, bool critical) const
    {
             ...
             AscensionCompatData::NativePowerGainRules)
        {
            ...
            player->ModifyPower(static_cast<Powers>(rule.PowerType),
                amount + NativePowerBonus(player, rule));
            changed = true;
        }
...
private:
    // Backswing (706790): "Increases the Runic Power generated by Reap by 5."
    // The native grants live in tenths, so the tooltip's 5 points are 50.
    static int32 NativePowerBonus(Player* player, AscensionCompatData::NativePowerGainRule const& rule)
    {
        if (player->getClass() == CLASS_REAPER && rule.PowerType == POWER_RUNIC_POWER &&
            player->HasAura(706790) &&
            sSpellMgr->GetFirstSpellInChain(rule.FirstSpellId) ==
                sSpellMgr->GetFirstSpellInChain(SPELL_REAPER_REAP))
            return 50;
        return 0;
    }
