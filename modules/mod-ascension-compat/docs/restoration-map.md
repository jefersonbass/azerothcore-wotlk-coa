...
lift that already exists in `mod-ascension-compat` is untouched: this is the map of what the
...
| `modules/mod-ascension-compat/src/AscensionCompat.cpp` | includes `LocalLevelScaling.h`; hosts the realm-wide fallback lift that must stay **off** while this module owns scaling |
...
| `modules/mod-ascension-compat/src/AscensionCompatOpcodes.h` | the client-opcode dispatch and the atlas ids it agrees with |
| `modules/mod-ascension-compat/docs/level-scaling.md` | the implementation reference (formulas, group rules, notifications, the hook-dispatch trap) |
| `modules/mod-ascension-compat/docs/level-scaling-vs-main.md` | the comparison against `main`, the answer on their fixes, the multi-player and notification audits |
...
* `CMSG_SET_LEVEL_SCALING` (0x0667) — the toggle the client can send; accepted by `mod-ascension-compat`'s
...
| `modules/mod-ascension-compat/src/AscensionCreaturePreset.cpp` | `creature_display_preset` — turns a preset row (customisation + apparel items) into the `CreatureDisplayInfoExtra` bake the client is sent, which is how the troll's gear is restored without a client patch |
| `modules/mod-ascension-compat/docs/npc-restoration.md` | the workflow: how a captured appearance is turned into preset + model + stream, for any NPC |
