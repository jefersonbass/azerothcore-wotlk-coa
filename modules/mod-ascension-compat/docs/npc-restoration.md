...
2. `UNIT_FLAG2_MIRROR_IMAGE = 0x00000010` (`UnitDefines.h`) marks the unit as one whose look comes
   over the wire; the client only asks for units that carry it, and `mod-ascension-compat` sets it
...
* Appearance and placement are independent: a correct preset on an unbuildable display is still an
  invisible NPC.
* `mod-ascension-compat` answers `0x0401` itself and returns `false`, so the core's clone-caster
