-- Dead proc gates on three classes, same shape as rev_20260921_53: each talent ships an aura-42 passive with
-- Spell.dbc ProcFlags 0 and no `spell_proc` row, so its clause never fired while the payload stays authored.
